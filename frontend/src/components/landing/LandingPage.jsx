...
export function LandingPage() {
  const [form, setForm] = useState({ username: 'demo_user', email: 'demo@meta.ai', password: 'MetaDiet123!', height_cm: 172, weight_kg: 74, goal_mode: 'rebuild' });
  ...
  const initialize = async () => {
    ...
    setError('');
    try {
      await api.post('/auth/signup/', { username: form.username, email: form.email, password: form.password }).catch(() => {});
      const login = await api.post('/auth/login/', { username: form.username, password: form.password });
      setToken(login.data.access);
      const scan = await api.post('/plans/scan/', { height_cm: Number(form.height_cm), weight_kg: Number(form.weight_kg), goal_mode: form.goal_mode });
      await new Promise(r => setTimeout(r, 550));
      setResult(scan.data);
    } catch (e) {
      setError(e?.response?.data?.detail || 'System initialization failed. Please retry.');
  ...
  return <div className="min-h-screen bg-[#050505] text-white px-4 py-5 sm:py-8 md:p-10 relative">
    ...
    <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.6}} className="relative z-10 max-w-6xl mx-auto glass panel p-5 sm:p-7 md:p-10 rounded-3xl shadow-neon cinematic-border">
      ...
      <p className="text-zinc-300 mt-4 max-w-2xl text-sm sm:text-base leading-relaxed">Control calories, metabolism, and performance from your personal Body Core System.</p>
      <div className="grid md:grid-cols-4 gap-3 mt-7">
        <input className="hud-input" placeholder="Username" value={form.username} onChange={e=>setForm({...form,username:e.target.value})} />
      ...
      {error && <p role="alert" className="mt-4 text-rose-300 text-sm">{error}</p>}
